...
function run_pytest() {
  ...
  if ! python3 -m pytest "${project_root_dir}" "$@" --cov=lib_github_actions; then
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : $HOME/venv/bin/lib_github_actions --version"
  if ! "$HOME/venv/bin/lib_github_actions" --version; then
