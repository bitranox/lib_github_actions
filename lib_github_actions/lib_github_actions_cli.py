...
try:
    ...
    from . import lib_github_actions
except (ImportError, ModuleNotFoundError):  # pragma: no cover
    # imports for doctest
    import __init__conf__  # type: ignore  # pragma: no cover
    import lib_github_actions  # type: ignore  # pragma: no cover
...
def cli_get_branch() -> None:
    ...
    response = lib_github_actions.get_branch()
    print(response)
...
def cli_run(
    ...
    lib_github_actions.run(
        description, command, retry=retry, sleep=sleep, banner=banner
    )
...
def cli_install(dry_run: bool) -> None:
    ...
    lib_github_actions.install(dry_run)
...
def cli_script(dry_run: bool) -> None:
    """updates pip, setuptools, wheel, pytest-pycodestyle"""
    lib_github_actions.script(dry_run)
...
def cli_after_success(dry_run: bool) -> None:
    """coverage reports"""
    lib_github_actions.after_success(dry_run)
...
def cli_deploy(dry_run: bool) -> None:
    """deploy on pypi"""
    lib_github_actions.deploy(dry_run)
